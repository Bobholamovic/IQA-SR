...
class ConcatResBlock(nn.Module):
    def __init__(
        ...
        _add_conv(n_feats, n_feats)
        _add_conv(n_feats, n_feats//2)
...
class MTEDSR(nn.Module):
    ...
    def forward(self, x):
        ...
        res_iqa = x_iqa = self.iqa_branch.head(x)

        for sr_blk, iqa_blk in zip(self.sr_branch[:-2], self.iqa_branch[:-2]):
            res_sr = sr_blk(res_sr, res_iqa)
            res_iqa = iqa_blk(res_iqa)

        # Do not share weights in the last two layers
        for idx in range(-2,0):
            res_sr = self.sr_branch[idx](res_sr)
            res_iqa = self.iqa_branch[idx](res_iqa)

        res_sr += x_sr
        res_iqa += x_iqa

        return self.sr_branch.tail(res_sr), self.iqa_branch.tail(res_iqa)
